  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (bounds) {
        ...
        centerDelta.copy(bounds.center).sub(lastCenter);
        const deltaLen = centerDelta.length();
        if (
          deltaLen > ROBOT_CAMERA_CENTERING_CONTINUOUS_DELTA_EPSILON_M &&
          !shouldSkipContinuousCameraFollowForBaseMotion(lastBasePose, currentBasePose)
        ) {
          // Keep camera and controls target offset stable; only follow robot centroid drift.
