...
export const EpisodeViewer3DModal: React.FC<EpisodeViewer3DModalProps> = ({
  ...
  const resetTrimAndFrame = useCallback((
    ...
    preservedFrameRef.current = nextFrame;
    setGlobalFrameRef.current?.(nextFrame);
  }, []);
  ...
  useEffect(() => {
    ...
    const runScan = async () => {
      if (robotClone) {
        ...
        if (constraintScanTokenRef.current !== scanToken) return;
        updateViewerFrame(restoreFrame);
        setGlobalFrameRef.current?.(restoreFrame);
        setCurrentFrame(restoreFrame);
        preservedFrameRef.current = restoreFrame;
      }
      setIsConstraintScanRunning(false);
    };
  ...
  }, [ensureTimelineEditingSession, jointNames, onSetGlobalFrame]);
  ...
  const handleTimelineMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    setCurrentFrame(frameIndex);
  }, [effectiveEpisode, onSetGlobalFrame, currentEpisodeIndex, allEpisodes, episode, onSetCurrentEpisodeIndex]);
  ...
  }, [effectiveEpisode, onSetGlobalFrame]);
  ...
  }, [isEditMode, editingJoint, modifiedEpisode, tangentHandles, jointRanges, selectedChartValueRange]);
