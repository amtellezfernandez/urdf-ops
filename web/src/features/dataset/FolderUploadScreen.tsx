...
import { URDF_OPS_ROUTE } from "@/shared/config/urdfOpsRoutes";
...
type LocalWebkitFile = File & { webkitRelativePath?: string };
...
type SourceOriginLabel = "folder" | "files" | "zip";

const normalizeLocalPath = (path: string): string => path.replace(/\\/g, "/").replace(/^\/+/, "");
...
const dedupePathsPreserveOrder = (paths: string[]): string[] => {
...
const dedupeUrdfCandidatesByPath = (candidates: URDFCandidate[]): URDFCandidate[] => {
...
const createOrderedLocalFileList = (files: File[], selectedPath: string): FileList => {
...
const deriveSelectedLocalFolder = (files: File[]): string | null => {
...
const cloneWithRelativePath = (file: File, nextRelativePath: string): File => {
...
const inferZipRoot = (paths: string[]): string | null => {
...
export const FolderUploadScreen = memo(
  ...
  const processPreparedLocalRobotSource = useCallback(
    async (preparedSource: PreparedLocalRobotSource): Promise<void> => {
      ...
      const localFiles = preparedSource.files;
      ...
      if (candidates.length === 0) {
        toast.error("No .urdf file found in the selected source");
        return;
      }
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (option.id === "training") {
                        ...
                        window.location.assign(URDF_OPS_ROUTE);
                        return;
                      }
