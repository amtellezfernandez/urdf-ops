import { Suspense, lazy, useState, useCallback, useMemo, startTransition, useEffect, useRef } from "react";
...
import {
  ...
  type FramePreflightResult,
  type GeneratePhysicsPreflightResult,
...
const formatSignedAxisLabel = (axis: "x" | "y" | "z", sign: 1 | -1): string =>
  `${sign > 0 ? "" : "-"}${axis.toUpperCase()}-up`;

const buildMeshFilesCacheKey = (meshFiles: Record<string, Blob>): string =>
  Object.entries(meshFiles)
    .sort(([leftPath], [rightPath]) => leftPath.localeCompare(rightPath))
    .map(([path, blob]) => `${path}:${blob.size}:${blob.type}`)
    .join("|");

const buildPackageRootsCacheKey = (packageRoots?: Record<string, string[]>): string =>
...
type PhysicsPreflightSession = GeneratePhysicsPreflightResult & {
  sourceContent: string;
  urdfBasePath?: string;
...
type FramePreflightSession = FramePreflightResult & {
  sourceContent: string;
};
...
const collectSynthesizedPhysicsLinkNames = (synthesisResult: InertialSynthesisResult): string[] => {
...
const WorldSceneImportDialog = lazy(() =>
...
const Index = () => {
  ...
  const initialCollaborationSession = useMemo(
    () =>
  ...
  const studioIssueReportUrl = useMemo(() => {
    if (!workspaceModeUi.showStudioIssueReport) return null;
    ...
    const lines = [
      ...
      `- Timestamp: ${nowIso}`,
      `- Mode: ${workspaceMode}`,
      `- active_urdf: ${urdfFile?.name ?? "none"}`,
      "",
      "### Issue",
  ...
  }, [workspaceMode, workspaceModeUi.showStudioIssueReport, urdfFile?.name]);
  ...
  const datasetReviewSessionId =
    datasetActions?.datasetSessionSummary?.session_id ?? null;
  ...
  const repeatedInertiaSymmetryLinkCentersLocal = useMemo(
    () =>
      new Map(
  ...
  const topNavBarProps: PageLayoutProps["topNavBarProps"] = {
    ...
    onOpenTrainingMode: () => navigate(URDF_OPS_ROUTE),
