...
def datasets_mix(req: DatasetMixRequest) -> DatasetMixResponse:
    """Submit a dataset mix job."""
    return mix_datasets(req)


@router.get("/mix/{job_id}", response_model=DatasetMixResponse)
def datasets_mix_job(job_id: str) -> DatasetMixResponse:
    """Fetch dataset mix job status."""
    return get_dataset_mix_job(job_id)
