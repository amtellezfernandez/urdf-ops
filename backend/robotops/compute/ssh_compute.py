...
class SSHDockerCompute:
    ...
    def __init__(
        self,
        output_dir: str = SSH_DEFAULT_OUTPUT_DIR,
        host: Optional[str] = None,
        user: Optional[str] = None,
        port: int = 22,
        key_path: Optional[str] = None,
        docker_image: str = SSH_DEFAULT_DOCKER_IMAGE,
        ...
        **_: Any,
        ...
        self.output_dir = output_dir.rstrip("/") or SSH_DEFAULT_OUTPUT_DIR
        self.docker_image = docker_image or SSH_DEFAULT_DOCKER_IMAGE
    ...
    async def _scp_to_remote(
        ...
        return await asyncio.to_thread(
            subprocess.run,
            [*self._scp_base(), str(local_path), f"{self._target}:{remote_path}"],
            capture_output=True,
            text=True,
            timeout=timeout,
            check=False,
    ...
    async def launch(
        ...
        job_id = f"ssh_{uuid.uuid4().hex[:SSH_JOB_ID_HEX_LENGTH]}"
        ...
        docker_args = " ".join(shlex.quote(arg) for arg in shlex.split(self.docker_args))
        container_script = (
            f"python /app/backend/scripts/train_policy.py --config /app/outputs/{shlex.quote(job_id)}/config.json "
            f"> /app/outputs/{shlex.quote(job_id)}/stdout.log "
            f"2> /app/outputs/{shlex.quote(job_id)}/stderr.log"
        )
        command = (
            ...
            f"sh -lc {shlex.quote(container_script)}"
        )
    ...
    async def status(self, job_id: str) -> JobStatus:
        ...
        inspect_cmd = (
            "docker inspect -f "
            f"{shlex.quote('{{.State.Status}} {{.State.ExitCode}} {{.State.Error}}')} "
    ...
    async def _read_remote_tail(self, remote_path: str, lines: int) -> Optional[str]:
        command = (
            ...
            f"tail -n {int(lines)} {shlex.quote(remote_path)} || true"
        )
    ...
    async def _read_remote_logs(self, remote_job_dir: str, lines: int) -> Optional[str]:
        stdout_tail = await self._read_remote_tail(f"{remote_job_dir}/stdout.log", lines)
    ...
    async def logs(self, job_id: str, follow: bool = False) -> AsyncIterator[str]:
        job = self._ensure_job(job_id)
        logs = await self._read_remote_logs(job["remote_job_dir"], 200)
        if logs:
            yield logs
    ...
    async def download_artifact(self, job_id: str, artifact_name: str, dest: Path) -> Path:
        raise FileNotFoundError("Remote artifact download is not wired to the API yet")
