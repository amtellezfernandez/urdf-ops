...
class TrainingApiModel(BaseModel):
    """Base model that accepts snake_case requests and emits camelCase UI payloads."""
...
class DatasetConfig(TrainingApiModel):
...
class ModelConfig(TrainingApiModel):
...
class TrainingParams(TrainingApiModel):
...
class TrackerConfig(TrainingApiModel):
...
class ComputeConfig(TrainingApiModel):
...
class TrainingStartRequest(TrainingApiModel):
...
class TrainingCancelRequest(TrainingApiModel):
...
class TrainingProgress(TrainingApiModel):
...
class TrainingMetrics(TrainingApiModel):
...
class TrainingLineage(TrainingApiModel):
...
class TrainingStartResponse(TrainingApiModel):
...
class TrainingJobSummary(TrainingApiModel):
...
class TrainingJobsListResponse(TrainingApiModel):
...
class TrainingRuntimeDependencyStatus(TrainingApiModel):
    """Local runtime dependency status."""

    name: str = Field(description="Python module name")
    required: bool = Field(description="Whether this module is required for local training")
    installed: bool = Field(description="Whether the module can be imported")
    version: Optional[str] = Field(default=None, description="Installed package version")
    error: Optional[str] = Field(default=None, description="Dependency resolution error")


class TrainingRuntimeCheckResponse(TrainingApiModel):
    """Local training runtime readiness check."""

    available: bool = Field(description="Whether local training can launch with required modules")
    python_executable: str = Field(description="Python executable used by the backend")
    dependencies: List[TrainingRuntimeDependencyStatus] = Field(default_factory=list)
    cuda_available: Optional[bool] = Field(default=None, description="Whether torch reports CUDA availability")
    message: str = Field(description="Human-readable runtime readiness summary")


class TrainingComputeBackendCapability(TrainingApiModel):
    """Readiness contract for a training compute backend."""

    type: str = Field(description="Compute backend identifier")
    label: str = Field(description="Human-readable backend name")
    enabled: bool = Field(description="Whether this backend can start production training jobs")
    production_ready: bool = Field(description="Whether execution, logs, artifacts, and cancellation are wired")
    reason: Optional[str] = Field(default=None, description="Why a backend is unavailable")
    missing_capabilities: List[str] = Field(
        default_factory=list,
        description="Lifecycle capabilities that must be added before this backend is enabled",
    )


class TrainingComputeBackendsResponse(TrainingApiModel):
    """Available training compute backends and their readiness."""

    backends: List[TrainingComputeBackendCapability] = Field(default_factory=list)
...
class ModelArchitectureInfo(TrainingApiModel):
...
class ModelsListResponse(TrainingApiModel):
...
class EvaluateRequest(TrainingApiModel):
...
class EpisodeResult(TrainingApiModel):
...
class EvaluateResponse(TrainingApiModel):
