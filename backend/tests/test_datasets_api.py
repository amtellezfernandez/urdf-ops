...
def test_datasets_mix_routes_submission_to_control_plane(monkeypatch: pytest.MonkeyPatch) -> None:
    expected = DatasetMixResponse(
        job_id="dataset-mix-123",
        status="queued",
    ...
    monkeypatch.setattr(datasets_api, "mix_datasets", lambda _req: expected)

    result = datasets_api.datasets_mix(
        DatasetMixRequest(
    ...
    assert result.job_id == "dataset-mix-123"
    assert result.status == "queued"


def test_datasets_mix_job_routes_status_lookup(monkeypatch: pytest.MonkeyPatch) -> None:
    expected = DatasetMixResponse(
        job_id="dataset-mix-123",
        status="succeeded",
        created_at="2026-04-09T10:00:00+00:00",
        updated_at="2026-04-09T10:02:00+00:00",
        completed_at="2026-04-09T10:02:00+00:00",
        success=True,
        message="done",
        output_path="/tmp/mixed",
    )
    monkeypatch.setattr(datasets_api, "get_dataset_mix_job", lambda _job_id: expected)

    result = datasets_api.datasets_mix_job("dataset-mix-123")

    assert result.status == "succeeded"
    assert result.output_path == "/tmp/mixed"
